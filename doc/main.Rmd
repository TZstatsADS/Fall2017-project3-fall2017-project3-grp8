...
packages.used=c("e1071","EBImage","OpenImageR","xgboost","ggplot2","gbm","tidyr")
...
```{r}
Baseline=T       #Use baseline or the Advance Model
...
if (run.cv){
  if (!Baseline){  ##We do CV for candidates in the Advanced Model
    ...
    par.list = list(cost = c(0.01, 0.1, 1, 5, 10),
                 gamma = c(0.01, 0.1, 1, 10, 100, 200, 450))
    ...
    ## parameters for svm_linear
    par.list = list(cost = c(0.1,1,3,5,8,10,50,100))
    res_linear <- cv.svm_linear(dat_train, par.list, K)
    svm_linear.para <- res_linear$best.par
    
    save(res_linear, file = "../output/svmLinear_err_cv.Rdata")
    # Print the Cross-Validation Error for the chosen Model:
    cat("The Cross Validation Error for svm_linear =", res_linear$smallest.cv_error,"\n")
    cat("The \'best\' cost for svm_linear =",svm_linear.para$cost,"\n")
    
    
    ### find cv-error for our advanced model
    n1 <- nrow(dat_train)
    n.fold <- floor(n1/K)
    s1 <- sample(rep(1:K, c(rep(n.fold, K-1), n-(K-1)*n.fold))) 
    cv_err.adv <- rep(NA, K)
    for (i in 1:K){
      ...
      fit.linear = train.svm_linear(train.data, svm_linear.para)
      pred.linear = test.svm_linear(fit.linear, data.test)
      
      pred = cbind(pred.xgb,pred.rbf,pred.linear)
...
if (!Baseline){
  if (run.cv){
    ...
    tm_train_lin <- system.time(fit_train_lin = train.svm_linear(dat_train, svm_linear.para))
  } else {
    ...
    svm_rbf.para1 <- list(cost=5, gamma=100)
    svm_linear.para1 <- list(cost = 100)
    ...
    tm_train_lin <- system.time(fit_train_lin = train.svm_linear(dat_train, svm_linear.para1))
  ...
  save(fit_train_lin, file="../output/fit_train_lin.RData")
}
...
if (!Baseline){
 ...
 pre_test.lin<-test.svm_linear(fit_train_lin, dat_test[,-1])
 com <- cbind(pre_test.xgb, pre_test.rbf, pre_test.lin)
...
if(run.test){
  ...
  if (Baseline){
    ...
    load(file="../output/fit_train_lin.RData") 
    ...
    tm_test_lin <- system.time(
           pred_new_lin <-test.svm_linear(fit_train_lin,new.X))
    
    tcom = cbind(pred_new_cgb,pred_new_rbf,pred_new_lin)
