...
if (run.cv){
  if (!Baseline){  ##We do CV for candidates in the Advanced Model
    ...
    for(k in 1:length(iter_values)){
      cat("k=", k, "\n")
      err_cv[k,] <- cv.function(dat_train, par=list(maxit=iter_values[k]), K)
    }
    ...
    for (i in 1:K){
      ...
      pred = cbind(pred.xgb,as.numeric(pred.rbf)-1,as.numeric(pred.linear)-1)
...
if (!Baseline){
 ...
 com <- cbind(pre_test.xgb, as.numeric(pre_test.rbf)-1, as.numeric(pre_test.lin)-1)
...
if(run.test){
  ...
  if (Baseline){
    ...
    tcom = cbind(pred_new_cgb,as.numeric(pred_new_rbf)-1,as.numeric(pred_new_lin)-1)
